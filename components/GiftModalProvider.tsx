...
type GiftModalContextValue = {
  openGiftModal: () => void;
  closeGiftModal: () => void;
};

const GiftModalContext = createContext<GiftModalContextValue | null>(null);

export function useGiftModal(): GiftModalContextValue {
  const context = useContext(GiftModalContext);
  if (!context) {
    throw new Error("useGiftModal must be used within GiftModalProvider");
  }
  return context;
}
          ...
          >
            ...
            <div className="mb-6 flex items-center gap-3 text-purple-rich">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Bank transfer details</p>
              </div>
            </div>
...
export function GiftModalProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <GiftModalContext.Provider value={{ openGiftModal, closeGiftModal }}>
      {children}
      {mounted && createPortal(<GiftModal open={open} onClose={closeGiftModal} />, document.body)}
